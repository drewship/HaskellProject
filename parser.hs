...
parseIf ::[ TokenPos ] -> Either String ( Statement, [ TokenPos ] )
parseIf tokens = do
  ( ifExp , afterExp ) <- parseF tokens
  case afterExp of
    (( TokSymb OpenBrace , _ ): afterBrace)  -> do
      (ifStmts , afterIfStmts ) <-  ifHelper afterBrace
      let ifStmt = StmtIf ifExp ifStmts
      case afterIfStmts of
        (( TokKeyword Else , _ ) : ( TokKeyword If , _ ): afterIf ) -> do
          ( nextElse , afterElse ) <- parseElseIf ( afterIf )
          let ifBlock = StmtIfBlock ((ifStmt:[]) ++ (nextElse))
          Right ( ifBlock ,afterElse)
        (( TokKeyword Else , _ ): afterElse ) -> do
          ( nextElse , afterElse ) <- parseElse (drop 1 afterIfStmts)
          let ifBlock = StmtIfBlock (((ifStmt:[]) ++ (nextElse:[])))
          Right ( ifBlock ,afterElse)
        _ -> Right (ifBlock , afterIfStmts)
          where
            ifBlock = StmtIfBlock ( ifStmt : [] )
    _ -> undefined


parseElseIf :: [ TokenPos ] -> Either String ( [Statement], [ TokenPos ] )
parseElseIf tokens = do
  ( elseIfExp , afterExp ) <- parseF tokens
  case afterExp of
    ( TokSymb OpenBrace , _ ): afterBrace -> do
      (elifStmts , afterElifStmts ) <-  ifHelper afterBrace
      let elifStmt = StmtElseIf elseIfExp elifStmts
      case afterElifStmts of
        (( TokKeyword Else , _ ) : ( TokKeyword If , _ ): afterIf ) -> do
          ( nextElse , afterElse ) <- parseElseIf (drop 2 afterElifStmts)
          Right ((elifStmt:[]) ++ (nextElse),afterElse)
        (( TokKeyword Else , _ ): afterElse ) -> do
          ( nextElse , afterElse ) <- parseElse (drop 1 afterElifStmts)
          Right (((elifStmt:[]) ++ (nextElse:[]),afterElse))


        _ -> Right ((elifStmt : [] ) , afterElifStmts)


    _ -> undefined


parseElse :: [ TokenPos ] -> Either String ( Statement, [ TokenPos ] )
parseElse ( first : rest ) =
  case first of
    ( TokSymb OpenBrace , _ ) -> do
      (elseStmt , afterStmts ) <- ifHelper rest
      Right (StmtElse elseStmt , afterStmts)
    _ -> ( Debug.Trace.trace $ "  " ++ show( first ) ++ "  ")undefined

ifHelper :: [ TokenPos ] -> Either String ( [Statement], [ TokenPos ] )
ifHelper [] = undefined
ifHelper tokens =
  case tokens of
    ( TokSymb CloseBrace, _ ) : afterBrace -> Right ( [], afterBrace )
